...
public class KThread {
	...
	//add
	if(currentThread.isJoin){
		KThread thread = currentThread.join.nextThread();
		while(thread != null){
			thread.ready();
			thread = currentThread.join.nextThread();
		}
	}
	...
	if(!this.isJoin){
		if(this != currentThread){ 
			isJoin = true;
			join.waitForAccess(currentThread); 
			currentThread.sleep(); 
		}
	}
	Machine.interrupt().enable();
    ...
    //add
    private ThreadQueue join = ThreadedKernel.scheduler.newThreadQueue(true);;
